...
df <- bind_rows(train,test) #appends the test dataset to the train dataset
dfActivities <- bind_rows(trainLabels,testLabels) #appends the test labels to the train labels
names(dfActivities) <- "activityID"
dfSubjects <- bind_rows(trainSubject,testSubject) #appends the test subjects to the train subjects
...
df <- bind_cols(select(df,contains("mean")),select(df,contains("std"))) #extracts only columns with mean and std in the name
...
df <- bind_cols(dfActivities,dfSubjects,df) #join activities to the main data frame
dfTidy <- df %>%
        select(-activityID) %>%
